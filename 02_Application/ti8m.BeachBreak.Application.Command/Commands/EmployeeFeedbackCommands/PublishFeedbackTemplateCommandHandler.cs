...
/// <summary>
/// Handler for PublishFeedbackTemplateCommand.
/// Publishes a feedback template with ownership validation.
/// </summary>
public class PublishFeedbackTemplateCommandHandler : ICommandHandler<PublishFeedbackTemplateCommand, Result>
{
    ...
    public PublishFeedbackTemplateCommandHandler(
    ...
    public async Task<Result> HandleAsync(PublishFeedbackTemplateCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            template.Publish(employeeId, userRole);
        ...
        {
            return Result.Fail($"Failed to publish feedback template: {ex.Message}", StatusCodes.Status500InternalServerError);
        }
    }
}
