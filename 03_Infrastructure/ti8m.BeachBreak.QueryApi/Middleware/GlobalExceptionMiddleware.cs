...
namespace ti8m.BeachBreak.QueryApi.Middleware;

/// <summary>
/// Global exception handling middleware for QueryApi.
...
public class GlobalExceptionMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        logger.LogError(exception,
            "Unhandled exception occurred in QueryApi. " +
    ...
    private (int statusCode, string message, string? details) GetErrorResponse(Exception exception)
    {
        return exception switch
        {
            ...
            ArgumentNullException => (400, "Required query parameters are missing", exception.Message),
            ArgumentException => (400, "Invalid query parameters", exception.Message),

            // Handle authorization exceptions
            UnauthorizedAccessException => (401, "Unauthorized access", "Authentication is required"),
            SecurityException => (403, "Access forbidden", "Insufficient permissions to access this resource"),

            // Handle data access exceptions specific to queries
            InvalidOperationException when exception.Message.Contains("sequence", StringComparison.OrdinalIgnoreCase)
                => (404, "Resource not found", "The requested data could not be found"),

            // Handle timeout exceptions (more common in queries)
            TaskCanceledException => (408, "Query timeout", "The query operation timed out"),
            TimeoutException => (408, "Query timeout", "The query operation timed out"),

            // Handle resource not found (common in query operations)
            ...
            // Handle query-specific data exceptions
            InvalidCastException => (500, "Data format error", "The requested data format is incompatible"),
            NotSupportedException when exception.Message.Contains("query", StringComparison.OrdinalIgnoreCase)
                => (400, "Unsupported query", exception.Message),

            // Default: Internal server error for unhandled exceptions
            _ => (500, "An internal error occurred", "The server encountered an unexpected condition while processing the query")
        };
    }
